...
async sendButtonsVid(jid, text, path, buttons = [], footer = "© KRIZSER", quoted, options) {
...
async sendButtonsGif(jid, text, path, buttons = [], footer = "© KRIZSER", quoted, options) {
            let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await getBuffer(path) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
...
async sendSticker(jid, media, quoted, opts = {}) {
   ...
   return await this.sendMessage(jid, await convert(medias.data, null, opts.packname || global.packname, opts.author || global.author), 'stickerMessage', { quoted: quoted, ...opts })
    ...
    reply(jid, text, quoted, options) {
      return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : this.sendMessage(jid, text, MessageType.extendedText, { contextInfo: { mentionedJid: this.parseMention(text),  externalAdReply: {title: 'ꪶ 𝑲𝑹𝑰𝒁⇥𝑺𝑬𝑹ꫂ⁩', body: '© Krishnadas', sourceUrl: '', thumbnail: global.thumb3}}, sendEphemeral: true, quoted, ...options })
    }
    /**
     * Fake Replies
     * @param {String} jid
